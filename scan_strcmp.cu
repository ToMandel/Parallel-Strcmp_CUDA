...
{
 __shared__ int diff[BLOCK_DIM];
    int tid = threadIdx.x;
    int stride = blockDim.x;
    int local_diff = 0;

    for (int i = tid; i < min(n1, n2); i += stride) {
        if (s1[i] != s2[i]) {
            local_diff = s1[i] - s2[i];
            break;
        }
    }
    ...
    // Compute prefix sum using scan_plus
    scan_plus(diff, BLOCK_DIM);
    __syncthreads();

    if (tid == 0) {
        *result = diff[BLOCK_DIM - 1];
    }
}
